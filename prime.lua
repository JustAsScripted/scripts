        ...
        repeat
			if (not (MyPlayer.Character.HumanoidRootPart:WaitForChild(nChild.Name).Velocity.Magnitude > 44 and MyPlayer.Character.HumanoidRootPart:WaitForChild(nChild.Name).Velocity.Magnitude < 46)) then
				...
				elseif (MyPlayer.Character.HumanoidRootPart:WaitForChild(nChild.Name).Velocity.Magnitude > 35) then
					local newVelocity = (MyPlayer.Character.HumanoidRootPart:WaitForChild(nChild.Name).Velocity + (MyPlayer.Character.HumanoidRootPart:WaitForChild(nChild.Name).Velocity))
...
function DefendShot()
	...
	local point = direction * (range + 4)
	local finalpos = startpos + point
	task.spawn(function()
		repeat
			MyPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(MyPlayer.Character.HumanoidRootPart.Position, Vector3.new(finalpos.X, MyPlayer.Character.HumanoidRootPart.Position.Y, finalpos.Z))
			task.wait()
		until MyPlayer:GetAttribute('UsingSkill')
	end)
	if (MyPlayer.Character.HumanoidRootPart.Position - finalpos).Magnitude <= 4 then
		if not MyPlayer.PlayerGui.SkilsGui.SlotThree.CDFrame.Visible then
			...
			until MyPlayer:GetAttribute('UsingSkill')
			wait(1)
		elseif not MyPlayer.PlayerGui.SkilsGui.SlotFive.CDFrame.Visible then
			repeat
				local args = {
					[1] = 'Hold',
					[2] = MyPlayer.PlayerGui.SkilsGui.SlotFive.SkillName.Text}
			...
			until MyPlayer:GetAttribute('UsingSkill')
			wait(1)
		elseif not MyPlayer.PlayerGui.SkilsGui.SlotFour.CDFrame.Visible then
			repeat
				local args = {
					[1] = 'Hold',
					[2] = MyPlayer.PlayerGui.SkilsGui.SlotFour.SkillName.Text}
				game:GetService('ReplicatedStorage').Remotes.TranciverRemote:FireServer(unpack(args))
				local args = {
					[1] = 'UseSkill',
					[2] = MyPlayer.PlayerGui.SkilsGui.SlotFour.SkillName.Text}
		...
		if not MyPlayer.PlayerGui.SkilsGui.SlotFour.CDFrame.Visible then
			wait(0.15)
			repeat
				local args = {
					[1] = 'Hold',
			...
			until MyPlayer:GetAttribute('UsingSkill')
			wait(1)
		elseif not MyPlayer.PlayerGui.SkilsGui.SlotThree.CDFrame.Visible then
			repeat
				local args = {
					[1] = 'Hold',
					[2] = MyPlayer.PlayerGui.SkilsGui.SlotThree.SkillName.Text}
			...
			until MyPlayer:GetAttribute('UsingSkill')
			wait(1)
		elseif getgenv().DefensiveMode and not MyPlayer.PlayerGui.SkilsGui.SlotSix.CDFrame.Visible then
			repeat
				local args = {
					[1] = 'Hold',
					[2] = MyPlayer.PlayerGui.SkilsGui.SlotSix.SkillName.Text}
		...
		if getgenv().IsAutoTrapping and getgenv().BallOwner ~= MyPlayer and not MyPlayer.Character:FindFirstChild('Ball') and getgenv().BallOwner ~= '' and game:GetService('Players'):FindFirstChild(getgenv().BallOwner.Name) and game:GetService('Players')[getgenv().BallOwner.Name].Character then
			for _, anim in pairs(getgenv().BallOwner.Character.Humanoid:GetPlayingAnimationTracks()) do
				...
				elseif anim.Animation.AnimationId == 'rbxassetid://12699056251' and anim.TimePosition > 0.1 then
					DefendShot()
				elseif anim.Animation.AnimationId == 'rbxassetid://12698894288' and getgenv().DefensiveMode then
					task.spawn(function()
						repeat
							MyPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(MyPlayer.Character.HumanoidRootPart.Position, Vector3.new(getgenv().RealBall.Ball.Position.X, MyPlayer.Character.HumanoidRootPart.Position.Y, getgenv().RealBall.Ball.Position.Z))
							task.wait()
						until MyPlayer:GetAttribute('UsingSkill')
					end)
					local args = {
...
Sections.Punch:AddLabel('Bicycle Kick'):AddKeyPicker('BicycleKickKeyBind', {
    Default = 'R',
    Mode = 'Toggle',
    NoUI = true,
    Callback = function(Value)
